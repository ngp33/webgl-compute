...
let edgesFBO = glComp.createFBO(
  ...
  new Float32Array(
    new Array(PARTICLES_W)
      .fill(0)
      .map((v, x) =>
        new Array(PARTICLES_H).fill(0).map((v, y) => {
          return [
            // Rest length = l
...
for (int i = 0; i < 8; i++) {
  ...
  acc += (0.0005 * ((length(AB) - (i >= 4 ? sqrt(2.0) : 1.0) * ${SPRING_LENGTH.toFixed(
    1
  )}) * normalize(AB)) +
...
const updatePos = glComp.createComputation(
  ...
  `
fragColor = clamp(${glComp.MACROS.fbo_idx_myxy(
    "pos"
  )} + ${glComp.MACROS.fbo_idx_myxy("vel")} * dt, 0.0, 100.0);
  `
);
