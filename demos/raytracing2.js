...
const intersectsFBO = glComp.createFBO(VIEWPORT_PIXELS, VIEWPORT_PIXELS, 4, "f32");
const allLightsFBO = glComp.createIndexedFBO([NUM_LIGHTS, VIEWPORT_PIXELS, VIEWPORT_PIXELS], 4, "f32");
const numSummationSteps = Math.log2(NUM_LIGHTS);
const summationFBOs = new Array(numSummationSteps)
    .fill(0)
    .map((_, i) => glComp.createIndexedFBO([2 ** (numSummationSteps - i - 1), VIEWPORT_PIXELS, VIEWPORT_PIXELS], 4, "f32"));
...
`);
const shadeAllLights = glComp.createComputation({ intersects: "fbo", t: "float", outputWidth: "int" }, 
/*GLSL*/ `
${glComp.MACROS.idx_from_2d_dynamic("my_idx", [String(NUM_LIGHTS), String(VIEWPORT_PIXELS), String(VIEWPORT_PIXELS)], ["outputWidth", ""], ["int(gl_FragCoord.x)", "int(gl_FragCoord.y)"])}
vec3 spherePos = vec3(0.0, 0.0, -2.0);
...
vec3 lightPos = spherePos + rotation * vec3(2.0 * sin(float(my_idx_0) * 3.14*2.0/${floatStr(NUM_LIGHTS)}), 1.0, 2.0 * cos(float(my_idx_0) * 3.14*2.0/${floatStr(NUM_LIGHTS)}));
...
fragColor = vec4(
  ...
  0.0);
`);
const sumStep = glComp.createComputation({ inputFbo: "fbo", inputNumLights: "int", outputWidth: "int" }, 
/*GLSL*/ `
${glComp.MACROS.idx_from_2d_dynamic("my_idx", ["(inputNumLights/2)", String(VIEWPORT_PIXELS), String(VIEWPORT_PIXELS)], ["outputWidth", ""], ["int(gl_FragCoord.x)", "int(gl_FragCoord.y)"])}

${glComp.MACROS.idx_to_2d_dynamic("idx_a", ["textureSize(inputFbo, 0).x", ""], ["(inputNumLights/2)", String(VIEWPORT_PIXELS), String(VIEWPORT_PIXELS)], ["(my_idx_0*2)", "my_idx_1", "my_idx_2"])}

${glComp.MACROS.idx_to_2d_dynamic("idx_b", ["textureSize(inputFbo, 0).x", ""], ["(inputNumLights/2)", String(VIEWPORT_PIXELS), String(VIEWPORT_PIXELS)], ["(my_idx_0*2 + 1)", "my_idx_1", "my_idx_2"])}

fragColor = texelFetch(inputFbo, idx_a, 0) + texelFetch(inputFbo, idx_b, 0);
  `);
const renderToCanvas = glComp.createComputation({ fbo: "fbo" }, 
